...
import {IHeroOFTXOperator, IKey} from "/interfaces/IHeroOFTXOperator.sol";
import {HeroOFTXCallbacksOperator} from "/HeroOFTXCallbacksOperator.sol";
import {TickerOperator} from "/TickerOperator.sol";
...
abstract contract HeroOFTXOperator is IHeroOFTXOperator, HeroOFTXCallbacksOperator, HeroOFTX, TickerOperator {
    ...
    function _validatorCrosschain(uint32 _lzDstEndpointId, address _to) internal virtual returns (uint256 totalMinted_) {
        ...
        try this.validatorLZSend(_lzDstEndpointId, _to, tokenIdOrAmount) {} catch (bytes memory) {
    ...
    function getActionsFeeTotal(
        uint32 _srcLzEndpoint,
        address _of,
        uint256[] calldata _indexes
    ) external view returns (uint256 amountDue_) {
        RequireAction[] storage allActions = requireActions[_srcLzEndpoint][_of];
        uint256 totalActions = allActions.length;
    ...
    function getPendingActions(uint32 _lzEndpointID, address _user) external view override returns (RequireAction[] memory) {
